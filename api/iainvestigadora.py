...
async def sb(method: str, table: str, *, params: dict[str, str] | None = None, payload: Any = None, prefer: str | None = None) -> Any:
    ...
    if response.status_code >= 300:
        raise HTTPException(status_code=422 if "23514" in response.text else 502, detail=f"SUPABASE_{table}_{response.status_code}:{response.text[:900]}")
...
async def rpc(name: str, payload: dict[str, Any]) -> Any:
    ...
    if response.status_code >= 300:
        raise HTTPException(status_code=422 if "23514" in response.text else 502, detail=f"SUPABASE_RPC_{name}_{response.status_code}:{response.text[:900]}")
    return response.json() if response.content else None
...
@app.get("/")
async def root():
    return {"agent_id":AGENT_ID,"agent_version":AGENT_VERSION,"system_version":SYSTEM_VERSION,"kernel_version":KERNEL_VERSION,"protocol_id":PROTOCOL_ID,"patch_sha256":PATCH_SHA256,"target_scope":"ONE_EXPLICIT_MLB_GAME_PER_RUN","workflow":list(PHASE_ORDER),"handoff_destination":"@ianalista","real_money_authority":REAL_MONEY_AUTHORITY}
...
async def health():
    ...
    return {"ok":bool(rows and rows[0].get("status")=="ACTIVE" and rows[0].get("protocol_id")==PROTOCOL_ID),"agent":rows[0] if rows else None}
...
async def start_run(req: RunStart):
    ...
    if not all(x.strip() for x in (req.game_id, req.away_team, req.home_team)):
    ...
    run = {"run_id":run_id,"system_version":SYSTEM_VERSION,"run_date":req.run_date,"status":"OPEN","mode":"CONTROLLED_REAL","metadata":{**req.metadata,"target_source":req.target_source,"requested_game_id":req.game_id}}
    saved_run = await sb("POST","runs",payload=run,prefer="return=representation")
    game = {"run_id":run_id,"game_id":req.game_id,"away_team":req.away_team,"home_team":req.home_team,"scheduled_start":req.scheduled_start,"cutoff_at":req.cutoff_at or req.scheduled_start,"status":"PREGAME_TARGET","decision_reason":{"venue":req.venue,"target_source":req.target_source}}
    try:
        saved_game = await sb("POST","games",payload=game,prefer="return=representation")
    except Exception:
        await sb("DELETE","runs",params={"run_id":f"eq.{run_id}"},prefer="return=minimal")
        raise
    return {"run":(saved_run or [run])[0],"target":(saved_game or [game])[0]}
...
async def create_tool_event(req: ToolEventCreate):
    if req.operation.upper() not in {"SEARCH","OPEN","FETCH","READ","QUERY","EXTRACT","OTHER"}:
        raise HTTPException(status_code=422, detail="INVALID_RESEARCH_OPERATION")
    row={"event_id":"PLACEHOLDER","run_id":req.run_id,"game_id":req.game_id,"tool_name":req.tool_name,"operation":req.operation.upper(),"request_hash":req.request_hash,"response_hash":req.response_hash,"source_ref":req.source_ref,"source_url":req.source_url,"material_new_info":req.material_new_info,"kernel_query_id":req.kernel_query_id,"retrieval_mode":req.retrieval_mode,"kernel_attested":True,"source_published_at":req.source_published_at,"data_available_since_kernel":req.data_available_since_kernel,"custody_version":"SEMANTIC-CUSTODY-1.0"}
...
async def create_evidence(req: EvidenceCreate):
    ...
    row={"evidence_id":"PLACEHOLDER","run_id":req.run_id,"game_id":req.game_id,"tool_name":req.tool_name,"source_ref":req.source_ref,"source_url":req.source_url,"retrieved_at":"1970-01-01T00:00:00Z","payload_hash":"PLACEHOLDER","payload":req.payload,"tool_event_id":req.tool_event_id,"original_publisher":req.original_publisher,"snapshot_hash":req.snapshot_hash,"claims_extracted":req.claims_extracted,"evidence_scope":"SPORTS_REASONING","factual_extract_text":req.factual_extract_text,"kernel_attested":True,"custody_version":"SEMANTIC-CUSTODY-1.0"}
...
async def submit_phase(req: PhaseSubmit):
    ...
    row={"run_id":req.run_id,"game_id":req.game_id,"protocol_id":PROTOCOL_ID,"phase_id":req.phase_id,"status":req.status,"payload":req.payload,"evidence_ids":req.evidence_ids,"source_calls":req.source_calls,"documents_analyzed":req.documents_analyzed,"output_text":req.output_text,"skip_reason":req.skip_reason}
...
async def register_report_document(req: ReportDocumentCreate):
    ...
    if not runs: raise HTTPException(status_code=404,detail="RUN_NOT_FOUND")
...
async def audit(run_id: str): return await rpc("iainv_derive_audit",{"p_run_id":run_id})


@app.post("/runs/{run_id}/close")
async def close(run_id: str): return await rpc("iainv_close_run",{"p_run_id":run_id})
