...
app = FastAPI(title="@NRFImetrica Mother V3 Final Report Gate", version="2.1")
...
async def latest_trace_hash(run_id: str) -> str | None:
    rows = await sb("GET", "trace_events", params={"select": "event_hash,occurred_at", "run_id": f"eq.{run_id}", "order": "occurred_at.desc", "limit": "1"}) or []
...
async def report_gate_status():
    return {
        ...
        "sports_reasoning_slate_required": True,
        "verified_drive_final_report_required_for_close": True,
...
async def validate_report(req: ReportRequest):
    runs = await sb("GET", "runs", params={"select": "run_id,status,mode,system_version", "run_id": f"eq.{req.run_id}", "limit": "1"}) or []
    if not runs:
    ...
    required = await sb(
        "GET", "protocol_run_state",
        params={
            "select": "stage_id,status,payload",
            "run_id": f"eq.{req.run_id}",
            "protocol_id": f"eq.{MOTHER_PROTOCOL_ID}",
            "stage_id": "in.(A0_CONSTITUTION_SEALED,SPORTS_REASONING_SLATE,A1_SLATE_ROUTED,A7_SLATE_ELIGIBILITY,A8_PORTFOLIO)",
        },
    ) or []
    completed = {row["stage_id"] for row in required if row.get("status") == "COMPLETE"}
    required_stages = ("A0_CONSTITUTION_SEALED", "SPORTS_REASONING_SLATE", "A1_SLATE_ROUTED", "A7_SLATE_ELIGIBILITY", "A8_PORTFOLIO")
    ...
    event = {
        ...
        "details": {"protocol_id": MOTHER_PROTOCOL_ID, "mother_document_sha256": MOTHER_DOCUMENT_SHA256, "document_ref": req.document_ref},
    }
    event["event_hash"] = stable_hash(event)
    await sb("POST", "trace_events", payload=event, prefer="return=minimal")

    drive = await sb("GET", "research_drive_artifacts", params={"select": "artifact_id,drive_file_id,content_hash,verified_at", "run_id": f"eq.{req.run_id}", "artifact_type": "eq.FINAL_REPORT", "limit": "1"}) or []
    close_authorized = bool(drive)
    return {
        "accepted": True,
        "run_id": req.run_id,
        "stage_id": "FINAL_REPORT",
        ...
        "drive_final_report_verified": bool(drive),
        "next_required": None if close_authorized else "REGISTER_VERIFIED_DRIVE_FINAL_REPORT_ARTIFACT",
