...
def _assert_distribution(
    payload: dict[str, Any], prefix: str, *, tolerance: float = 1e-6
) -> tuple[float, float, float, float]:
    values = tuple(
        _number(_get_path(payload, f"{prefix}.{name}"), f"{prefix}.{name}")
        for name in ("p0", "p1", "p2", "p3plus")
    )
...
def _mother_semantics(phase_id: str, payload: dict[str, Any], output_text: str) -> None:
    ...
    if phase_id == "A1_DATA_INTEGRITY_FREEZE":
        if str(_get_path(payload, "market_quarantine")).upper() not in {
            "PASS",
            "SEALED",
            "INTACT",
            "ACTIVE",
        ...
        if str(_get_path(payload, "freeze_integrity")).upper() != "PASS":
            raise ProtocolViolation("A1_FREEZE_INTEGRITY_FAIL")
        ...
        p0, p1, p2, _ = _assert_distribution(payload, "joint")
        ...
        if str(_get_path(payload, "raw_not_calibrated_check")).upper() != "PASS":
            raise ProtocolViolation("A5_RAW_CALIBRATION_BOUNDARY_FAIL")
        if str(_get_path(payload, "market_blindness")).upper() != "PASS":
            raise ProtocolViolation("A5_MARKET_BLINDNESS_FAIL")
        ...
        target = str(_get_path(payload, "target_id")).upper()
        if target not in {"U0.5", "NRFI"}:
            raise ProtocolViolation("A7_CORE_TARGET_MUST_BE_ZERO_RUNS")
        if press_effect not in {
            "CONFIRM",
            "STRENGTHEN",
            "CONDITION",
            "REVISE",
        ...
        if release == "ISSUED":
            ...
            zero_cal_status = str(_get_path(payload, "contract_calibration.u0_5.status")).upper()
            if zero_cal_status not in {"CERTIFIED", "CERTIFIED_CONDITIONED"}:
                raise ProtocolViolation("A7_ZERO_RUN_CALIBRATION_REQUIRED_FOR_RELEASE")
            _number(
                _get_path(payload, "contract_calibration.u0_5.calibrated_p"),
                "contract_calibration.u0_5.calibrated_p",
            )
            _number(
                _get_path(payload, "contract_calibration.u0_5.conservative_p"),
                "contract_calibration.u0_5.conservative_p",
            )
        ...
        if str(_get_path(payload, "a7_eligibility_status")).upper() not in {
            "A7_ELIGIBLE",
            "A7_ELIGIBLE_CONDITIONED",
        }:
            raise ProtocolViolation("A8_ELIGIBILITY_BLOCKED")
        if str(_get_path(payload, "calibration_status")).upper() not in {
            "CERTIFIED",
            "CERTIFIED_CONDITIONED",
        }:
        ...
        p_conservative = _number(
            _get_path(payload, "market.p_conservative"), "market.p_conservative"
        )
        try:
...
def validate_phase_submission(
    *,
    ...
    missing_prereq = [
        p for p in phase.get("prerequisites", []) if p not in completed_phase_ids
    ]
    if missing_prereq:
    ...
    missing_fields = [
        path
        for path in phase.get("required_fields", [])
    ...
    required_docs = {
        str(x).strip() for x in phase.get("required_documents", []) if str(x).strip()
    }
    ...
    if required_docs:
        ...
        if untraced_docs:
            raise ProtocolViolation(
                "REQUIRED_DOCUMENTS_WITHOUT_REAL_TRACE:" + ",".join(untraced_docs)
            )
