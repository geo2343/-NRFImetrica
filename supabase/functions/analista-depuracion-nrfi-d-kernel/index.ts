...
function decodeJwt(auth:string|null){
  if(!auth?.startsWith("Bearer ")) throw new Error("AUTHORIZATION_BEARER_REQUIRED");
  const token=auth.slice(7); const parts=token.split("."); if(parts.length<2) throw new Error("JWT_INVALID");
  const raw=parts[1].replace(/-/g,"+").replace(/_/g,"/"); const pad=raw+"=".repeat((4-raw.length%4)%4);
  const claims=JSON.parse(atob(pad)); const role=String(claims.role||"");
  if(!["authenticated","service_role"].includes(role)) throw new Error("JWT_ROLE_NOT_ALLOWED");
  return {sub:claims.sub?String(claims.sub):null,role};
}
...
function blockExternalProof(action:string,p:any){
  ...
  if(a==="REGISTER_EVIDENCE" && String(p?.evidence_type||"")==="TOOL_READBACK") throw new Error("TOOL_READBACK_EVIDENCE_MUST_COME_FROM_EXTERNAL_CONNECTOR_HOST");
}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    const actor=decodeJwt(req.headers.get("authorization"));
    const body=await req.json(); const action=String(body.action||"").toLowerCase(); const p=(body.payload&&typeof body.payload==="object")?body.payload:{};
    ...
    if(action==="create_run"){
      const result=await rpc("depurnrfi_d_create_run_v11",{p_run_id:String(p.run_id||""),p_slate_date:p.slate_date,p_input_manifest:p.input_manifest||{},p_invocation_message_hash:String(p.invocation_message_hash||""),p_reasoning_host:REASONING_HOST});
      await rpc("depurnrfi_d_bind_actor_v11",{p_run_id:String(p.run_id||""),p_actor_sub:actor.sub,p_actor_role:actor.role});
      return Response.json({ok:true,result,next_action:"GET_EXECUTION_PLAN_AND_CONTINUE_AUTONOMOUSLY"});
    }
    ...
    const runId=getRunId(p); if(!runId) return Response.json({ok:false,error:"RUN_ID_REQUIRED"},{status:400});
    await rpc("depurnrfi_d_assert_actor_v11",{p_run_id:runId,p_actor_sub:actor.sub,p_actor_role:actor.role});
    ...
    if(action==="command"){
      const env=p.envelope||{}; const ep=env.payload||{}; blockExternalProof(String(env.action||""),ep);
      return Response.json({ok:true,result:await rpc("depurnrfi_d_submit_command_v11",{p_envelope:env})});
    }
    ...
    const m=String((e as any)?.message||e); const code=/AUTH|JWT|ACTOR|EXTERNAL_CONNECTOR|SELF_CERTIFIABLE/.test(m)?403:/STALE/.test(m)?409:422;
    return Response.json({ok:false,agent_id:AGENT,version:VERSION,error:m},{status:code});
  }
});
