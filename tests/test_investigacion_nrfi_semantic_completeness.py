...
class InvestigacionNRFISemanticCompletenessTests(unittest.TestCase):
    ...
    def test_semantic_snapshot_false_cannot_close(self):
        snapshot = {
            "f1_pass": True,
            "f2_pass": False,
            "f3_pass": True,
        ...
        with self.assertRaisesRegex(InvestigacionNRFIProtocolViolation, "SEMANTIC_COMPLETENESS_FAIL"):
            validate_semantic_snapshot(snapshot)

    def test_report_contract_blocks_short_or_missing_game_blocks(self):
